...
**Comparativa visual (red fuerte vs red débil, mismo amortiguamiento).** La figura siguiente reúne, con los valores reales del ejemplo (proyecto 04) y el mismo \( R_d \) fijado en red fuerte sin retocar, todo lo derivado en los Pasos 3–6 de una sola vista:

<div class="cfig"><img src="figuras/filtro-lcl-red-vs-aislado.png" alt="Cuatro paneles comparando red fuerte y red debil: (a) Bode de i2/vi con el pico de resonancia que baja poco de 3404 a 2995 Hz; (b) Bode de i1/vi con el valle de antiresonancia que baja mucho de 2034 a 1233 Hz; (c) barras de fres, far, Q y Leq; (d) atenuacion k a fsw para L2 sola, red fuerte y red debil"><div class="cap">(a) \( |i_2/v_i| \): el pico de resonancia se desplaza poco a la izquierda (\( f_{res} \): 3404→2995 Hz, \( -12\% \)) — y con el mismo \( R_d \) el pico de red débil queda relativamente más alto (peor amortiguado, coherente con \( Q_{ef} \) un 14 % mayor). (b) \( |i_1/v_i| \): el valle de antiresonancia baja mucho más (\( f_{ar} \): 2034→1233 Hz, \( -39\% \)), justo lo que predice el Paso 4 (\( f_{ar}\propto1/\sqrt{L_{2ef}} \) sin saturar, frente a \( f_{res}\propto1/\sqrt{L_{eq,ef}} \) que satura hacia \( L_1 \)). (c) las cuatro magnitudes clave en escala log: \( Q \) sube de 3.0 a 3.4 (\( \times1.14 \), Paso 5). (d) \( k \) a \( f_{sw} \): de \( 37\% \) con \( L_2 \) sola a \( 4.1\% \) (red fuerte) y \( 1.5\% \) (red débil) — la mejora de \( \times9 \) y \( \times24 \) del Paso 6, real pero no controlable.</div></div>
...
## 9 — Diseño completo paso a paso (proceso iterativo real)
Los ocho apartados anteriores dan cada fórmula por separado. Pero un diseño real no es aplicarlas una vez en orden: es aplicarlas, **comprobar que el resultado de una no estropea la hipótesis de otra**, y recalcular cuando sí lo hace. Este apartado recorre un caso completo —10 kVA, 400 V (\( V_{ll} \)), 50 Hz, \( V_{dc}=700 \) V, \( f_{sw}=10 \) kHz— mostrando los recálculos tal como aparecen, no una secuencia limpia inventada a posteriori. Las fórmulas usadas son exactamente las finales (encuadradas) de cada apartado.

**Datos de partida (se calculan una vez).** Corriente de pico nominal de fase \( I_n=\sqrt2\,S_n/(\sqrt3\,V_{ll})=20.4 \) A; tensión rms de fase \( V_{rms}=V_{ll}/\sqrt3=230.9 \) V; bandas objetivo: \( \zeta\in[0.1,0.3] \) (apartado 3), \( r=L_2/L_1\in[0.2,1] \) (apartado 7), \( f_{res} \) holgada en \( 10f_0<f_{res}<f_{sw}/2 \), es decir \( 500<f_{res}<5000 \) Hz.

**Iteración 0 — aplicar cada apartado por separado (el camino ingenuo).**

- \( L_1 \) por rizado (apartado 5), fijando \( \Delta i_{1,amp}=15\% \) de \( I_n \): \( L_1=V_{dc}/(8f_{sw}\Delta i_{1,amp})=700/(8\cdot10^4\cdot0.15\cdot20.4)=2.86 \) mH.
- \( C_f \) por reactiva al máximo del 5 % (apartado 6): \( C_f=0.05\,S_n/(\omega_0 V_{rms}^2)=29.8\,\mu\text{F} \).
- \( L_2 \) por atenuación (apartado 7), con \( k=10\% \) (centro del rango típico): \( L_2=1/(k\,C_f\,\omega_{sw}^2)=0.085 \) mH.

Comprobación de \( r \) (apartado 7): \( r=L_2/L_1=0.085/2.86=\mathbf{0.030} \). **Está fuera de \( [0.2,1] \) por casi un orden de magnitud.** Además \( f_{res}=3209 \) Hz y \( f_{ar}=3162 \) Hz quedan casi pegadas (\( f_{res}/f_{ar}=\sqrt{1+r}=\sqrt{1.03}\approx1.015 \), solo 1.5 % de separación): el cero de antiresonancia que estabiliza el lazo de \( i_1 \) (apartado 1, Paso 6) prácticamente se solapa con la resonancia y deja de ser útil. Hay que recalcular.

**Iteración 1 — primer recálculo: subir \( L_2 \) para meter \( r \) en rango.** \( L_1 \) está fijado por el rizado y no se toca. La palanca es \( L_2 \): se fija el objetivo \( r=0.30 \) (dentro del rango) y se despeja \( L_2=0.30\,L_1=0.86 \) mH. Pero \( L_2 \) y \( C_f \) comparten la fórmula de \( k \) (apartado 7) y la de la reactiva (apartado 6) — subir \( L_2 \) con el \( C_f \) de antes dispararía \( k \) en sentido equivocado, así que el recálculo correcto es **bajar \( C_f \)** para reconciliar ambos: con \( C_f=20\,\mu\text{F} \) (\( \approx3.4\% \) de \( S_n \) en reactiva, holgadamente bajo el 5 %), el mismo \( L_2=0.86 \) mH da \( k=1/(\omega_{sw}^2L_2C_f)=\mathbf{1.5\%} \). Es decir: al forzar \( r \) a entrar en rango, la atenuación **mejora** muy por debajo del 10 % pedido (consecuencia, no objetivo — es el efecto que ya anticipaba el apartado 7 al hablar de la utilidad de \( r \)).

Ahora \( r=0.30 \) ✓ y \( f_{res}=1386 \) Hz, dentro de la banda \( 500\text{–}5000 \) ✓. Pero falta la verificación que el apartado 8 marca como obligatoria: **el comportamiento en red débil.**

**Iteración 2 — segundo recálculo: re-amortiguar tras fallar en red débil.** Se dimensiona el amortiguamiento pasivo óptimo para el caso aislado (apartado 3): \( R_d=1/(3\omega_{res}C_f)=1.91\,\Omega \), que da \( \zeta=0.167 \) ✓ en red fuerte. Aplicando la regla de verificación del apartado 8 con una red de SCR≈10 (\( L_g\approx5.1 \) mH para esta potencia/tensión), la sustitución \( L_2\to L_{2ef}=L_2+L_g \) baja \( f_{res} \) a 810 Hz y, con el **mismo \( R_d \) sin retocar**, \( \zeta \) cae a \( \mathbf{0.097} \) — por debajo del mínimo de 0.1. El amortiguamiento diseñado para la red fuerte ya no basta justo donde más hace falta.

El recálculo: redimensionar \( R_d \) al \( f_{res} \) del **peor caso** (red débil), \( R_d=1/(3\omega_{res,ef}C_f)=3.27\,\Omega \). Con ese valor, \( \zeta=0.167 \) ✓ en red débil y \( \zeta=0.285 \) ✓ en red fuerte — **ambos extremos dentro de \( [0.1,0.3] \)**, así que el diseño converge: un solo \( R_d \) cubre todo el rango de red. (Si no hubiera convergido, el siguiente recálculo habría sido bajar \( f_{res} \) resituando \( L_2 \) o pasar a amortiguamiento activo \( K_{ad} \), que no depende de \( R_d \) fijo.)

**Resultado y lectura del proceso.** La figura siguiente traza el recorrido: cada conflicto, qué se recalcula y qué se hereda sin tocar entre iteraciones.

<div class="cfig"><img src="figuras/filtro-lcl-diseno-iterativo.png" alt="Cuatro paneles del diseno iterativo: (a) plano r-k con It0 fuera de rango y It1 dentro; (b) barras de fres bajando en cada paso dentro de la banda; (c) barras de zeta con It1 cayendo bajo 0.1 en red debil y It2 corrigiendolo; (d) tabla resumen de las tres iteraciones"><div class="cap">(a) Conflicto 1: It.0 (rojo) cae fuera del rango de \( r \); subir \( L_2 \) lleva a It.1 (azul) dentro, mejorando \( k \) de paso. (b) \( f_{res} \) se desplaza en cada paso pero se mantiene en la banda \( 500\text{–}5000 \) Hz (la red débil la baja de 1386 a 810 Hz). (c) Conflicto 2: con el \( R_d \) de It.1, \( \zeta \) cae bajo 0.1 en red débil (barra roja); re-amortiguar (It.2) devuelve ambos escenarios a la banda \( [0.1,0.3] \). (d) recorrido completo en tabla: \( L_1 \) se hereda intacto, \( C_f \) baja una vez (It.0→It.1), \( R_d \) sube una vez (It.1→It.2); la columna final (verde) es el diseño que cumple todo a la vez.</div></div>

**Por qué importa hacerlo así.** Si se hubiera aplicado la lista de fórmulas una sola vez (Iteración 0), el filtro habría salido con \( r=0.03 \) (antiresonancia inútil) y, de haber seguido, con \( \zeta=0.097 \) en red débil (resonancia mal amortiguada en el peor caso) — dos defectos que ninguna fórmula individual señala porque cada una, por sí sola, da un resultado "correcto". Los conflictos solo aparecen al cruzar el resultado de un apartado con la hipótesis de otro, y por eso el diseño es iterativo por naturaleza, no una receta lineal.
