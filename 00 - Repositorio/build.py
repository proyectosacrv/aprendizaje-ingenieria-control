...
/* ── Pantalla completa: ocultar la cabecera ──────────────────────── */
.hidebtn{background:var(--panel2);border:1px solid var(--line);color:var(--ink);
  padding:7px 12px;border-radius:8px;font-size:13px;cursor:pointer}
.hidebtn:hover{border-color:var(--acc)}
body.fs header{display:none}
body.fs .wrap{min-height:100vh}
body.fs .list,body.fs .detail{max-height:100vh}
body.fs #graph{height:100vh}
.fsbtn{position:fixed;top:8px;right:10px;z-index:60;display:none;
  background:var(--panel2);border:1px solid var(--line);color:var(--muted);
  border-radius:20px;padding:7px 13px;font-size:13px;cursor:pointer;opacity:.9}
.fsbtn:hover{color:var(--ink);border-color:var(--acc)}
body.fs .fsbtn{display:inline-block}
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  const setFS=v=>{document.body.classList.toggle('fs',v);try{localStorage.setItem('fs',v?'1':'0')}catch(e){}};
  $('#fshide').onclick=()=>setFS(true);
  $('#fsshow').onclick=()=>setFS(false);
  setFS(localStorage.getItem('fs')==='1');
  render(); applyHash();
});
...
<header>
  ...
  <div class="bar">
    ...
    <button class="hidebtn" id="fshide" title="Ocultar el menú (pantalla completa)" aria-label="Ocultar el menú">⤢ Pantalla completa</button>
  </div>
</header>
<button class="fsbtn" id="fsshow" title="Mostrar el menú" aria-label="Mostrar el menú">≡ menú</button>
<div class="wrap">
